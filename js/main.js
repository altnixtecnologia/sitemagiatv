...
async function getMatches() {
    ...
    try {
        ...
        mainContainer.innerHTML = `<p class="text-center text-red-500">Sincronizando com o painel...</p>`;
    }
}
    ...
    container.innerHTML = matches.map(match => `
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500 card-hover text-center">
            ...
            <div class="flex items-center justify-between gap-4 mb-4">
                <div class="flex-1 text-sm font-bold">${match.teams.home.name}</div>
                <div class="text-gray-300 font-black">VS</div>
                <div class="flex-1 text-sm font-bold">${match.teams.away.name}</div>
            </div>
            <button class="w-full bg-green-600 text-white py-2 rounded-lg text-sm font-bold" onclick="window.open('${API_CONFIG.wysterExternal}', '_blank')">Assistir no Painel</button>
        </div>`).join('');
}
...
function renderMovies() {
    ...
    container.innerHTML = MOVIE_HIGHLIGHTS.map(movie => `
        <div class="bg-white rounded-lg shadow-md overflow-hidden group">
            ...
            <div class="p-4 text-center font-bold text-sm">${movie.title}</div>
        </div>`).join('');
}
